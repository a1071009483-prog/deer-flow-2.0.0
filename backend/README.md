...
Phoenix runs on a DeerFlow-owned `TracerProvider`, separate from the host global OTel provider. Registration uses `set_global_tracer_provider=false`, `batch=true`, `auto_instrument=false`, and `shutdown_on_exit=false`; DeerFlow saves the returned provider and then explicitly initializes a single `LangChainInstrumentor` and binds the manual `deerflow.run` boundary to it. If a host already owns the LangChain instrumentor, DeerFlow leaves it unchanged and guarantees only manual `deerflow.run` spans on its provider. Only a DeerFlow-owned LangChain instrumentor is uninstrumented during shutdown. DeerFlow never enumerates `openinference_instrumentor` entry points, never inspects or restores foreign instrumentor state, and never reads or writes `OPENINFERENCE_*` environment variables. Standard `OTEL_BSP_MAX_QUEUE_SIZE`, `OTEL_BSP_SCHEDULE_DELAY`, `OTEL_BSP_EXPORT_TIMEOUT`, and `OTEL_BSP_MAX_EXPORT_BATCH_SIZE` variables configure batching.

Gateway shutdown drains in-flight runs before Phoenix cleanup. It executes `force_flush` followed by `shutdown` on a daemon thread with the gateway's five-second wait deadline and does not touch the provider's private `atexit` hook. This keeps exporter latency off the event loop and prevents a timed-out cleanup from blocking interpreter exit. Per-run graph and embedded stream scopes end spans and restore context but never shut down the process provider.
...
When `PHOENIX_CAPTURE_CONTENT=false`, DeerFlow filters only the attributes exported by the Phoenix-owned OpenInference instrumentor and manual run span. The `RunnableConfig.metadata` used by LangGraph, tools, skills, LangSmith, Langfuse, and custom callbacks is not filtered or replaced. `PHOENIX_METADATA_ALLOWLIST` defaults to empty and accepts comma-separated, exact top-level caller metadata keys, with whitespace removed and duplicate keys ignored in first-seen order. The example exports only `request_id` and `tenant_id`; all other caller metadata and every caller tag remain excluded from the Phoenix export view. Provider-reserved allowlist entries, including `langfuse_*`, are ignored by the Phoenix export filter. DeerFlow's session/thread, user, assistant/subagent, model, environment, root run name, run id, and controlled tags have final precedence. The allowlist controls Phoenix export only and is not an authorization mechanism; treat allowlisted values as untrusted unless a gateway or RBAC layer generates or validates them. When `PHOENIX_CAPTURE_CONTENT=true`, full invocation metadata and tags can be exported; only enable it for trusted workloads.
