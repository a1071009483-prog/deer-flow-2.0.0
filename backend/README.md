...
Delegation authorization is bound when each agent is built. Every agent gets a distinct `task` tool closure carrying a `DelegationParentContext` that atomically binds the immutable parent policy and resolved parent model name; SDK factory callers pass that identity through `create_deerflow_agent(..., parent_model_name=...)`. There is no process-global unrestricted task tool, and `RunnableConfig.metadata.model_name` is not an authorization source. The resolver and `SubagentExecutor` consume the same bound parent model. A single fail-closed resolver validates the parent policy and child request, resolves the exact tool/skill set, and passes that immutable decision to `SubagentExecutor`. `None` means unrestricted. An empty `tool_groups` collection permits no configured tool groups but does not disable independently governed built-in, MCP, or ACP tools; an empty `available_skills` collection denies all skills. Unknown names and incomplete configured/MCP/ACP/skill discovery deny delegation before execution.
