...
- `DelegationParentContext` atomically binds the immutable `DelegationPolicy` and resolved parent model name into the `task` tool closure when each agent is built. `create_deerflow_agent(..., parent_model_name=...)` carries that trusted model identity through SDK factory paths. Never restore a module-global `task_tool` or an unrestricted fallback when `subagent_enabled=True`.
- `resolve_delegation()` is the only component that interprets parent/child allowlists. `None` means unrestricted. An empty `tool_groups` collection means no configured tool groups are permitted; it does not disable independently governed built-in, MCP, or ACP tools. An empty `available_skills` collection denies all skills. The resolver loads one strict catalog snapshot, rejects unknown or incomplete state, applies configured groups, child allow/deny, and skill `allowed_tools`, then returns `ResolvedDelegation`.
...
- `RunnableConfig.metadata.model_name` is not an authorization input: the resolver and executor both consume the parent model bound in `DelegationParentContext`. `tool_groups` and `available_skills` are business policy and must never be written to or read from tracing metadata. Phoenix enablement and content-capture settings must not change the resolved model, tools, or skills.
